...
pixel_map = binned_deviations.statistic.T / binned_signs.statistic.T
pixel_map = np.nan_to_num(pixel_map)
pixel_map = np.abs(pixel_map)

image = mp.imshow(
    pixel_map, extent=[-RANGE, RANGE, -RANGE, RANGE], vmin=0.0, vmax=0.2)
mp.colorbar(image)

mp.savefig('src/processing/output/deviations.png', dpi=300)
